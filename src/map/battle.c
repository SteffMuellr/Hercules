...
static int battle_delay_damage(int64 tick, int amotion, struct block_list *src, struct block_list *target, int attack_type, uint16 skill_id, uint16 skill_lv, int64 damage, enum damage_lv dmg_lv, int ddelay, bool additional_effects)
{
	...
	if (((d_tbl && sc && check_distance_bl(target, d_tbl, sc->data[SC_DEVOTION]->val3)) || e_tbl) && damage > 0
#ifndef RENEWAL
	...
	switch(attack_type){
		case BF_MAGIC:
			switch(skill_id){
				...
				case WZ_VERMILION:
					...
					else { // Monsters continue to use the old formula: 100% 105% 115% 130% 150% 175% 205% 240% 280% 330% per 10 hits
		...
		case BF_WEAPON:
			switch( skill_id )
			{
				...
				case CH_PALMSTRIKE:
#ifdef RENEWAL
					skillratio += 100 + 100 * skill_lv + status_get_str(src) * 5;
					RE_LVL_DMOD(100);
#else
					skillratio += 100 + 100 * skill_lv;
#endif
...
static int64 battle_calc_damage(struct block_list *src, struct block_list *bl, struct Damage *d, int64 damage, uint16 skill_id, uint16 skill_lv)
{
	...
	if( sc && sc->count )
	{
		...
		if( sc->data[SC_WHITEIMPRISON] ) { // Pre-re: Gravitation & Pressure have  already returned. Re: They deal normal magic damage
...
#ifdef RENEWAL
				int atk = (3 + sc->data[SC_AURABLADE]->val1) * status->get_lv(src);
				atk *= ((skill_id == LK_SPIRALPIERCE || skill_id == ML_SPIRALPIERCE || skill_id == KN_BOWLINGBASH)?wd.div_:1); // Add hits*atk to skill's damages
				ATK_ADD(atk);
#else
...
static enum damage_lv battle_weapon_attack(struct block_list *src, struct block_list *target, int64 tick, int flag)
{
	...
	if (tsc && tsc->data[SC_BLADESTOP_WAIT]
		...
		&& !is_boss(src)
#endif
	&& (src->type == BL_PC || tsd == NULL || distance_bl(src, target) <= (tsd->weapontype == W_FIST ? 1 : 2)) )
	{
		uint16 skill_lv = tsc->data[SC_BLADESTOP_WAIT]->val1;
		int duration = skill->get_time2(MO_BLADESTOP,skill_lv);
#ifdef RENEWAL
		if (is_boss(src))
			duration = 2000; // only lasts 2 seconds on boss monsters
#endif
